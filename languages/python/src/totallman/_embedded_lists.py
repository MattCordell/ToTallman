# GENERATED by languages/python/tools/embed_lists.py
# Source: tallman-lists/compiled/lists.compiled.json
# DO NOT EDIT

from __future__ import annotations

from totallman.exceptions import TallmanError

ALL_LISTS: dict[str, dict[str, str]] = {
    "AU": {
        "aclin": "aCLin",
        "actonel": "actoNEL",
        "actos": "actoS",
        "akamin": "aKAMin",
        "aldactone": "alDACTONE",
        "aldomet": "alDOMET",
        "alkeran": "ALKeran",
        "alodorm": "alODORM",
        "amaryl": "amARYl",
        "aminophylline": "amiNOPHYLLine",
        "amiodarone": "amiODAROne",
        "amitriptyline": "amiTRIPTYLine",
        "amlodipine": "amLODIPine",
        "amoxil": "amOXil",
        "apomine": "aPomine",
        "arabloc": "arABLOC",
        "aratac": "arATAC",
        "aripiprazole": "ARIPiprazole",
        "aropax": "arOPAX",
        "atropt": "aTRopt",
        "avastin": "avaSTIN",
        "avaxim": "avaXIM",
        "avomine": "aVomine",
        "azathioprine": "azATHIOPRINE",
        "azithromycin": "aziTHROMYCIN",
        "azopt": "aZopt",
        "bisacodyl": "bisACODYl",
        "bisoprolol": "bisOPROLOl",
        "budesonide": "buDESONide",
        "bumetanide": "buMETANide",
        "caltrate": "caLTRate",
        "captopril": "caPTOPRil",
        "carafate": "caRAFate",
        "carbamazepine": "CARBAMazepine",
        "carbimazole": "carbiMAZOLe",
        "carboplatin": "cARBOplatin",
        "carpine": "CARpine",
        "carvedilol": "caRVEDILOl",
        "cefalexin": "cefaLEXin",
        "cefalotin": "cefALOTIN",
        "cefazolin": "cefaZOLin",
        "cefepime": "cefEPIME",
        "cefotaxime": "cefOTAXIME",
        "cefoxitin": "cefOXITIN",
        "ceftaroline": "ceftAROLine",
        "ceftazidime": "cefTAZIDIME",
        "ceftriaxone": "cefTRIAXONE",
        "celapram": "celAPRAM",
        "celebrex": "celEBREX",
        "cephalexin": "cephaLEXin",
        "cephazolin": "cephaZOLin",
        "chlorpromazine": "cHLORPROMAZine",
        "ciclosporin": "ciclosPORIN",
        "cipramil": "ciprAMIL",
        "ciprofloxacin": "ciPROFLOXAcin",
        "ciproxin": "ciprOXIN",
        "cisplatin": "ciSplatin",
        "clarithromycin": "cLARITHROMYcin",
        "clomiphene": "cLOMIPHEne",
        "clomipramine": "cLOMIPRAMine",
        "clonazepam": "CLONazepam",
        "contin": "Contin",
        "coumadin": "coUMADIN",
        "coversyl": "coVERSYL",
        "cyclophosphamide": "CYCLOPHOSPHamide",
        "cycloserine": "cyclosERINE",
        "cyclosporin": "cyclosPORIN",
        "dactinomycin": "daCTINomycin",
        "daptomycin": "daPTomycin",
        "daunorubicin": "DAUNOrubicin",
        "depo-medrol": "DEPO-medrol",
        "depo-provera": "depo-PROVERA",
        "deptran": "dePTRAn",
        "deralin": "deRALin",
        "diazepam": "Diazepam",
        "dilantin": "dilaNTIN",
        "dilaudid": "dilaUDID",
        "diprivan": "diPRIVan",
        "dipyridamole": "diPYRIDAMOLe",
        "disopyramide": "diSOPYRAMIDe",
        "ditropan": "diTROPan",
        "docetaxel": "DOCEtaxel",
        "dosulepin": "doSULepin",
        "dothiepin": "doTHiepin",
        "doxepin": "doXepin",
        "doxorubicin": "DOXOrubicin",
        "duloxetine": "DULoxetine",
        "erythromycin": "ERYthromycin",
        "fluoxetine": "fluoxetine",
        "fluvoxamine": "fluVOXAMine",
        "glibenclamide": "gliBENCLAMide",
        "gliclazide": "gliCLAZide",
        "glimepiride": "gliMEPIRide",
        "glipizide": "gliPIZide",
        "homatropine": "HOMATROpine",
        "humalog": "humALOG",
        "humulin": "humULIN",
        "hydralazine": "hydrALAZINe",
        "hydrea": "hydreA",
        "hydrene": "hydreNE",
        "hydrochlorothiazide": "hydrOCHLOROTHIAZIDe",
        "hydromorphone": "HYDROmorphone",
        "idarubicin": "iDArubicin",
        "ifosfamide": "iFOSFamide",
        "inderal": "INDEral",
        "isopto carpine": "isopto CARpine",
        "isopto homatropine": "isopto HOMATROpine",
        "isotretinoin": "iSOtretinoin",
        "janumet": "januMET",
        "januvia": "januVIA",
        "ketalar": "ketALAR",
        "ketorolac": "ketOROLAC",
        "lamictal": "laMICTAl",
        "lamisil": "laMISil",
        "lamivudine": "lamiVUDine",
        "lamotrigine": "lamOTRIGine",
        "lantus": "lanTUs",
        "lanvis": "lanVis",
        "largactil": "laRGACTil",
        "leukeran": "LEUKeran",
        "lincomycin": "linCOMYCIN",
        "linezolid": "linEZOLID",
        "lipazil": "lipAZil",
        "lipidil": "lipiDil",
        "lorazepam": "LORazepam",
        "losec": "loSEC",
        "lovan": "loVAN",
        "mercaptamine": "mercaptAMine",
        "mercaptopurine": "mercaptOPURine",
        "methadone": "methADONe",
        "methylphenidate": "methYLPHENIDATe",
        "mobilis": "moBILis",
        "morphine": "morphine",
        "movalis": "moVALis",
        "moxifloxacin": "MOXifloxacin",
        "ms": "MS",
        "myleran": "MYLeran",
        "neoral": "NEOral",
        "nexavar": "nexAVAR",
        "nexium": "nexiUM",
        "nifedipine": "niFEDIPine",
        "nimodipine": "niMODIPine",
        "nizatidine": "niZATIDine",
        "norfloxacin": "NORfloxacin",
        "normison": "norMISON",
        "norvasc": "norVASC",
        "novomix": "novoMIX",
        "novorapid": "novoRAPID",
        "oxazepam": "OXazepam",
        "oxcarbazepine": "OXCARBazepine",
        "oxycontin": "oxyCONTIN",
        "oxynorm": "oxyNORM",
        "paclitaxel": "PACLitaxel",
        "pariet": "paRIET",
        "paroxetine": "PARoxetine",
        "paxtine": "paXTINE",
        "pexsig": "pEXSIG",
        "primacin": "primaCIN",
        "primacor": "primaCOR",
        "primaxin": "primaXIN",
        "pristiq": "pRISTIQ",
        "prochlorperazine": "proCHLORPERazine",
        "prograf": "proGRAF",
        "promethazine": "proMETHazine",
        "propofol": "propOFol",
        "propranolol": "propRANOLol",
        "prozac": "proZAC",
        "quetiapine": "QUETIAPine",
        "rabeprazole": "RABEprazole",
        "rifampicin": "rifaMPICin",
        "rifaximin": "rifaXIMin",
        "risperidone": "riSPERIDONe",
        "ropinirole": "rOPINIROLe",
        "saxagliptin": "sAXagliptin",
        "sertraline": "SERTRALine",
        "sirolimus": "Sirolimus",
        "sitagliptin": "siTagliptin",
        "solu-cortef": "solu-CORTEF",
        "solu-medrol": "SOLU-medrol",
        "sulfadiazine": "sulfaDiazine",
        "sulfasalazine": "sulfaSALazine",
        "sumatriptan": "sUMATRIPTAn",
        "tacrolimus": "TACrolimus",
        "tapentadol": "tAPENTadol",
        "tegretol": "tEGRETOl",
        "temodal": "tEMOdal",
        "tofranil": "toFRANIL",
        "topamax": "toPAMAX",
        "toradol": "tORadol",
        "tramadol": "tRAMadol",
        "trental": "tRENTAl",
        "tretinoin": "tretinoin",
        "trimeprazine": "trimEPRAZINE",
        "trimethoprim": "trimETHOPRIM",
        "valaciclovir": "valAciclovir",
        "valganciclovir": "valGANciclovir",
        "vinblastine": "vinBLASTine",
        "vincristine": "vinCRISTine",
        "vinorelbine": "vinORELBine",
        "xalacom": "xalaCOM",
        "xalatan": "xalaTAN",
        "zinnat": "zinNAt",
        "zinvit": "zinVit",
        "zocor": "zoCOR",
        "zoloft": "zoLOFT",
        "zoton": "zoTON",
    },
    "DEFAULT": {
        "aclin": "aCLin",
        "actonel": "actoNEL",
        "actos": "actoS",
        "akamin": "aKAMin",
        "aldactone": "alDACTONE",
        "aldomet": "alDOMET",
        "alimemazine (trimeprazine)": "alimemazine (trimEPRAZINE)",
        "alkeran": "ALKeran",
        "alodorm": "alODORM",
        "amaryl": "amARYl",
        "aminophylline": "amiNOPHYLLine",
        "amiodarone": "amiODAROne",
        "amitriptyline": "amiTRIPTYLine",
        "amlodipine": "amLODIPine",
        "amoxil": "amOXil",
        "apomine": "aPomine",
        "arabloc": "arABLOC",
        "aratac": "arATAC",
        "aripiprazole": "ARIPiprazole",
        "aropax": "arOPAX",
        "atropt": "aTRopt",
        "avastin": "avaSTIN",
        "avaxim": "avaXIM",
        "avomine": "aVomine",
        "azathioprine": "azATHIOPRINE",
        "azithromycin": "aziTHROMYCIN",
        "azopt": "aZopt",
        "bisacodyl": "bisACODYl",
        "bisoprolol": "bisOPROLOl",
        "budesonide": "buDESONide",
        "bumetanide": "buMETANide",
        "caltrate": "caLTRate",
        "captopril": "caPTOPRil",
        "carafate": "caRAFate",
        "carbamazepine": "CARBAMazepine",
        "carbimazole": "carbiMAZOLe",
        "carboplatin": "cARBOplatin",
        "carvedilol": "caRVEDILOl",
        "cefalexin": "cefaLEXin",
        "cefalotin": "cefALOTIN",
        "cefazolin (cephazolin)": "cefaZOLin (cephaZOLin)",
        "cefepime": "cefEPIME",
        "cefotaxime": "cefOTAXIME",
        "cefoxitin": "cefOXITIN",
        "ceftaroline": "ceftAROLine",
        "ceftazidime": "cefTAZIDIME",
        "ceftriaxone": "cefTRIAXONE",
        "celapram": "celAPRAM",
        "celebrex": "celEBREX",
        "cephalexin": "cephaLEXin",
        "chlorpromazine": "cHLORPROMAZine",
        "ciclosporin": "ciclosPORIN",
        "cipramil": "ciprAMIL",
        "ciprofloxacin": "ciPROFLOXAcin",
        "ciproxin": "ciprOXIN",
        "cisplatin": "ciSplatin",
        "clarithromycin": "cLARITHROMYcin",
        "clomiphene": "cLOMIPHEne",
        "clomipramine": "cLOMIPRAMine",
        "clonazepam": "CLONazepam",
        "coumadin": "coUMADIN",
        "coversyl": "coVERSYL",
        "cyclophosphamide": "CYCLOPHOSPHamide",
        "cycloserine": "cyclosERINE",
        "cyclosporin": "cyclosPORIN",
        "dactinomycin": "daCTINomycin",
        "daptomycin": "daPTomycin",
        "daunorubicin": "DAUNOrubicin",
        "depo-medrol": "DEPO-medrol",
        "depo-provera": "depo-PROVERA",
        "deptran": "dePTRAn",
        "deralin": "deRALin",
        "diazepam": "Diazepam",
        "dilantin": "dilaNTIN",
        "dilaudid": "dilaUDID",
        "diprivan": "diPRIVan",
        "dipyridamole": "diPYRIDAMOLe",
        "disopyramide": "diSOPYRAMIDe",
        "ditropan": "diTROPan",
        "docetaxel": "DOCEtaxel",
        "dosulepin (dothiepin)": "doSULepin (doTHiepin)",
        "doxepin": "doXepin",
        "doxorubicin": "DOXOrubicin",
        "duloxetine": "DULoxetine",
        "erythromycin": "ERYthromycin",
        "fluoxetine": "fluoxetine",
        "fluvoxamine": "fluVOXAMine",
        "glibenclamide": "gliBENCLAMide",
        "gliclazide": "gliCLAZide",
        "glimepiride": "gliMEPIRide",
        "glipizide": "gliPIZide",
        "humalog": "humALOG",
        "humulin": "humULIN",
        "hydralazine": "hydrALAZINe",
        "hydrea": "hydreA",
        "hydrene": "hydreNE",
        "hydrochlorothiazide": "hydrOCHLOROTHIAZIDe",
        "hydromorphone": "HYDROmorphone",
        "idarubicin": "iDArubicin",
        "ifosfamide": "iFOSFamide",
        "inderal": "INDEral",
        "isopto carpine": "isopto CARpine",
        "isopto homatropine": "isopto HOMATROpine",
        "isotretinoin": "iSOtretinoin",
        "janumet": "januMET",
        "januvia": "januVIA",
        "ketalar": "ketALAR",
        "ketorolac": "ketOROLAC",
        "lamictal": "laMICTAl",
        "lamisil": "laMISil",
        "lamivudine": "lamiVUDine",
        "lamotrigine": "lamOTRIGine",
        "lantus": "lanTUs",
        "lanvis": "lanVis",
        "largactil": "laRGACTil",
        "leukeran": "LEUKeran",
        "lincomycin": "linCOMYCIN",
        "linezolid": "linEZOLID",
        "lipazil": "lipAZil",
        "lipidil": "lipiDil",
        "lorazepam": "LORazepam",
        "losec": "loSEC",
        "lovan": "loVAN",
        "mercaptamine (cysteamine)": "mercaptAMine (cysteamine)",
        "mercaptopurine": "mercaptOPURine",
        "methadone": "methADONe",
        "methylphenidate": "methYLPHENIDATe",
        "mobilis": "moBILis",
        "morphine": "morphine",
        "movalis": "moVALis",
        "moxifloxacin": "MOXifloxacin",
        "ms contin": "MS Contin",
        "myleran": "MYLeran",
        "neoral": "NEOral",
        "nexavar": "nexAVAR",
        "nexium": "nexiUM",
        "nifedipine": "niFEDIPine",
        "nimodipine": "niMODIPine",
        "nizatidine": "niZATIDine",
        "norfloxacin": "NORfloxacin",
        "normison": "norMISON",
        "norvasc": "norVASC",
        "novomix": "novoMIX",
        "novorapid": "novoRAPID",
        "oxazepam": "OXazepam",
        "oxcarbazepine": "OXCARBazepine",
        "oxycontin": "oxyCONTIN",
        "oxynorm": "oxyNORM",
        "paclitaxel": "PACLitaxel",
        "pariet": "paRIET",
        "paroxetine": "PARoxetine",
        "paxtine": "paXTINE",
        "pexsig": "pEXSIG",
        "prednisone": "predniSONE",
        "primacin": "primaCIN",
        "primacor": "primaCOR",
        "primaxin": "primaXIN",
        "pristiq": "pRISTIQ",
        "prochlorperazine": "proCHLORPERazine",
        "prograf": "proGRAF",
        "promethazine": "proMETHazine",
        "propofol": "propOFol",
        "propranolol": "propRANOLol",
        "prozac": "proZAC",
        "quetiapine": "QUETIAPine",
        "rabeprazole": "RABEprazole",
        "rifampicin": "rifaMPICin",
        "rifaximin": "rifaXIMin",
        "risperidone": "riSPERIDONe",
        "ropinirole": "rOPINIROLe",
        "saxagliptin": "sAXagliptin",
        "sertraline": "SERTRALine",
        "sirolimus": "Sirolimus",
        "sitagliptin": "siTagliptin",
        "solu-cortef": "solu-CORTEF",
        "solu-medrol": "SOLU-medrol",
        "sulfadiazine": "sulfaDiazine",
        "sulfasalazine": "sulfaSALazine",
        "sumatriptan": "sUMATRIPTAn",
        "tacrolimus": "TACrolimus",
        "tapentadol": "tAPENTadol",
        "tegretol": "tEGRETOl",
        "temodal": "tEMOdal",
        "tofranil": "toFRANIL",
        "topamax": "toPAMAX",
        "toradol": "tORadol",
        "tramadol": "tRAMadol",
        "trental": "tRENTAl",
        "tretinoin": "tretinoin",
        "trimethoprim": "trimETHOPRIM",
        "valaciclovir": "valAciclovir",
        "valganciclovir": "valGANciclovir",
        "vinblastine": "vinBLASTine",
        "vincristine": "vinCRISTine",
        "vinorelbine": "vinORELBine",
        "xalacom": "xalaCOM",
        "xalatan": "xalaTAN",
        "zinnat": "zinNAt",
        "zinvit": "zinVit",
        "zocor": "zoCOR",
        "zoloft": "zoLOFT",
        "zoton": "zoTON",
    },
    "FDA": {
        "acetazolamide": "acetaZOLAMIDE",
        "acetohexamide": "acetoHEXAMIDE",
        "bupropion": "buPROPion",
        "buspirone": "busPIRone",
        "chlorpromazine": "chlorproMAZINE",
        "chlorpropamide": "chlorproPAMIDE",
        "clomiphene": "clomiPHENE",
        "clomipramine": "clomiPRAMINE",
        "cycloserine": "cycloSERINE",
        "cyclosporine": "cycloSPORINE",
        "daunorubicin": "DAUNOrubicin",
        "dimenhydrinate": "dimenhyDRINATE",
        "diphenhydramine": "diphenhydrAMINE",
        "dobutamine": "DOBUTamine",
        "dopamine": "DOPamine",
        "doxorubicin": "DOXOrubicin",
        "glipizide": "glipiZIDE",
        "glyburide": "glyBURIDE",
        "hydralazine": "hydrALAZINE",
        "hydromorphone": "HYDROmorphone",
        "hydroxyzine": "hydrOXYzine",
        "medroxyprogesterone": "medroxyPROGESTERone",
        "methylprednisolone": "methylPREDNISolone",
        "methyltestosterone": "methylTESTOSTERone",
        "mitoxantrone": "mitoXANTRONE",
        "nicardipine": "niCARdipine",
        "nifedipine": "NIFEdipine",
        "prednisolone": "prednisoLONE",
        "prednisone": "predniSONE",
        "risperidone": "risperiDONE",
        "ropinirole": "rOPINIRole",
        "sulfadiazine": "sulfADIAZINE",
        "sulfisoxazole": "sulfiSOXAZOLE",
        "tolazamide": "TOLAZamide",
        "tolbutamide": "TOLBUTamide",
        "vinblastine": "vinBLAStine",
        "vincristine": "vinCRIStine",
    },
    "ISMP": {
        "alprazolam": "ALPRAZolam",
        "amiloride": "aMILoride",
        "amlodipine": "amLODIPine",
        "aripiprazole": "ARIPiprazole",
        "avinza": "AVINza",
        "azacitidine": "azaCITIDine",
        "azathioprine": "azaTHIOprine",
        "carbamazepine": "carBAMazepine",
        "carboplatin": "CARBOplatin",
        "cefazolin": "ceFAZolin",
        "cefotetan": "cefoTEtan",
        "cefoxitin": "cefOXitin",
        "ceftazidime": "cefTAZidime",
        "ceftriaxone": "cefTRIAXone",
        "celebrex": "CeleBREX",
        "celexa": "CeleXA",
        "chlordiazepoxide": "chlordiazePOXIDE",
        "chlorpromazine": "chlorproMAZINE",
        "cisplatin": "CISplatin",
        "clobazam": "cloBAZam",
        "clonazepam": "clonazePAM",
        "clonidine": "cloNIDine",
        "clozapine": "cloZAPine",
        "dactinomycin": "DACTINomycin",
        "daptomycin": "DAPTOmycin",
        "depo-medrol": "DEPO-Medrol",
        "diazepam": "diazePAM",
        "diltiazem": "dilTIAZem",
        "docetaxel": "DOCEtaxel",
        "doxorubicin": "DOXOrubicin",
        "duloxetine": "DULoxetine",
        "ephedrine": "ePHEDrine",
        "epinephrine": "EPINEPHrine",
        "epirubicin": "epiRUBicin",
        "eribulin": "eriBULin",
        "fentanyl": "fentaNYL",
        "flavoxate": "flavoxATE",
        "fluoxetine": "FLUoxetine",
        "fluphenazine": "fluPHENAZine",
        "fluvoxamine": "fluvoxaMINE",
        "guaifenesin": "guaiFENesin",
        "guanfacine": "guanFACINE",
        "humalog": "HumaLOG",
        "humulin": "HumuLIN",
        "hydralazine": "hydrALAZINE",
        "hydrochlorothiazide": "hydroCHLOROthiazide",
        "hydrocodone": "HYDROcodone",
        "hydromorphone": "HYDROmorphone",
        "hydroxyprogesterone": "HYDROXYprogesterone",
        "hydroxyzine": "hydrOXYzine",
        "idarubicin": "IDArubicin",
        "idarucizumab": "idaruCIZUmab",
        "infliximab": "inFLIXimab",
        "invanz": "INVanz",
        "isotretinoin": "ISOtretinoin",
        "klonopin": "KlonoPIN",
        "lamictal": "LaMICtal",
        "lamisil": "LamISIL",
        "lamivudine": "lamiVUDine",
        "lamotrigine": "lamoTRIgine",
        "leucovorin": "leucovorin",
        "levetiracetam": "levETIRAcetam",
        "levocarnitine": "levOCARNitine",
        "levofloxacin": "levoFLOXacin",
        "levoleucovorin": "LEVOleucovorin",
        "lorazepam": "LORazepam",
        "medroxyprogesterone": "medroxyPROGESTERone",
        "metformin": "metFORMIN",
        "methazolamide": "methazolAMIDE",
        "methimazole": "methIMAzole",
        "metolazone": "metOLazone",
        "metronidazole": "metroNIDAZOLE",
        "metyrapone": "metyraPONE",
        "metyrosine": "metyroSINE",
        "mifepristone": "miFEPRIStone",
        "misoprostol": "miSOPROStol",
        "mitomycin": "mitoMYcin",
        "mitoxantrone": "mitoXANTRONE",
        "morphine": "morphine",
        "nexavar": "NexAVAR",
        "nexium": "NexIUM",
        "nicardipine": "niCARdipine",
        "nifedipine": "NIFEdipine",
        "nimodipine": "niMODipine",
        "novolin": "NovoLIN",
        "novolog": "NovoLOG",
        "olanzapine": "OLANZapine",
        "oxcarbazepine": "OXcarbazepine",
        "oxycodone": "oxyCODONE",
        "oxycontin": "OxyCONTIN",
        "oxymorphone": "oxyMORphone",
        "paclitaxel": "PACLitaxel",
        "paroxetine": "PARoxetine",
        "pazopanib": "PAZOPanib",
        "pemetrexed": "PEMEtrexed",
        "penicillamine": "penicillAMINE",
        "penicillin": "penicillin",
        "pentobarbital": "PENTobarbital",
        "phenobarbital": "PHENobarbital",
        "ponatinib": "PONATinib",
        "pralatrexate": "PRALAtrexate",
        "prilosec": "PriLOSEC",
        "prozac": "PROzac",
        "quetiapine": "QUEtiapine",
        "quinidine": "quiNIDine",
        "quinine": "quiNINE",
        "rabeprazole": "RABEprazole",
        "ranitidine": "raNITIdine",
        "rifampin": "rifAMPin",
        "rifaximin": "rifAXIMin",
        "rimantadine": "riMANTAdine",
        "risperdal": "RisperDAL",
        "risperidone": "risperiDONE",
        "rituximab": "riTUXimab",
        "romidepsin": "romiDEPsin",
        "romiplostim": "romiPLOStim",
        "ropinirole": "rOPINIRole",
        "sandimmune": "SandIMMUNE",
        "sandostatin": "SandoSTATIN",
        "saxagliptin": "sAXagliptin",
        "seroquel": "SEROquel",
        "sinequan": "SINEquan",
        "sitagliptin": "SITagliptin",
        "solu-cortef": "Solu-CORTEF",
        "solu-medrol": "SOLU-Medrol",
        "sorafenib": "SORAfenib",
        "sufentanil": "SUFentanil",
        "sulfadiazine": "sulfADIAZINE",
        "sulfasalazine": "sulfaSALAzine",
        "sumatriptan": "SUMAtriptan",
        "sunitinib": "SUNItinib",
        "tegretol": "TEGretol",
        "tiagabine": "tiaGABine",
        "tizanidine": "tiZANidine",
        "tramadol": "traMADol",
        "trazodone": "traZODone",
        "trental": "TRENtal",
        "tretinoin": "tretinoin",
        "valacyclovir": "valACYclovir",
        "valganciclovir": "valGANciclovir",
        "zolmitriptan": "ZOLMitriptan",
        "zyprexa": "ZyPREXA",
        "zyrtec": "ZyrTEC",
    },
    "NZ": {
        "alkeran": "ALKeran",
        "allopurinol": "aLLOPURINol",
        "aminophylline": "amINOPHYLLIne",
        "amiodarone": "amIODAROne",
        "amisulpiride": "amiSULPIRIDe",
        "amitriptyline": "amITRIPTYLIne",
        "amlodipine": "amLODIPIne",
        "apomine": "aPomine",
        "aratac": "arATAC",
        "aropax": "arOPAX",
        "atenolol": "aTENOLol",
        "atropt": "aTRopt",
        "avastin": "avaSTIN",
        "avaxim": "avaXIM",
        "avomine": "aVomine",
        "azathioprine": "azATHIOPRINE",
        "azithromycin": "azITHROMYCIN",
        "azopt": "aZopt",
        "benzathine benzylpenicillin": "BENZATHINE benzylpenicillin",
        "benzylpenicillin": "benzylpenicillin",
        "bisacodyl": "bisACODYl",
        "bisoprolol": "bisOPROLOl",
        "budesonide": "buDESONide",
        "bumetanide": "buMETANide",
        "caltrate": "caLTRate",
        "captopril": "caPTOPRIl",
        "carafate": "caRAFate",
        "carbamazepine": "CARBAMazepine",
        "carbimazole": "carbIMAZOLe",
        "carboplatin": "cARBOplatin",
        "carvedilol": "caRVEDILOl",
        "cefalexin": "cefaLEXin",
        "cefazolin": "cefaZOLin",
        "cefepime": "cefEPIME",
        "cefotaxime": "cefOTAXIME",
        "cefoxitin": "cefOXITIN",
        "ceftazidime": "cefTAZIDIME",
        "ceftriaxone": "cefTRIAXONE",
        "cefuroxime": "cefUROXIME",
        "celapram": "celAPRAM",
        "celebrex": "celEBREX",
        "cephalexin": "cephaLEXin",
        "cephazolin": "cephaZOLin",
        "cetuximab": "CETUximab",
        "chlorpromazine": "cHLORPROMAZIne",
        "cipramil": "ciprAMIL",
        "ciprofloxacin": "cIPROFLOXAcin",
        "ciproxin": "ciprOXIN",
        "cisplatin": "cISplatin",
        "clarithromycin": "cLARITHROMYcin",
        "clindamycin": "clINDAmycin",
        "clomiphene": "cLOMIPHEne",
        "clomipramine": "cLOMIPRAMIne",
        "clonazepam": "CLONazepam",
        "clonidine": "cloNIDine",
        "clozapine": "cLOZAPine",
        "coumadin": "coUMADIN",
        "coversyl": "coVERSYL",
        "cyclizine": "cyclIZINE",
        "cycloblastin": "cyclOBLASTIN",
        "cyclophosphamide": "CYCLOPHOSPHamide",
        "cycloserine": "cyclosERINE",
        "cyclosporin": "cyclosPORIN",
        "dactinomycin": "daCTINomycin",
        "daptomycin": "daPTomycin",
        "daunorubicin": "DAUNOrubicin",
        "depo-medrol": "DEPO-medrol",
        "depo-provera": "depo-PROVERA",
        "diazepam": "DIazepam",
        "digoxin": "dIGOXin",
        "dipyridamole": "diPYRIDAMOLe",
        "disopyramide": "diSOPYRAMIDe",
        "docetaxel": "DOCEtaxel",
        "dothiepin": "doTHIEpin",
        "doxazosin": "dOXAZOSin",
        "doxepin": "doXEpin",
        "doxorubicin": "DOXOrubicin",
        "duloxetine": "DULoxetine",
        "erythromycin": "ERYthromycin",
        "fluarix": "fluARIx",
        "fluoxetine": "fluoxetine",
        "flupenthixol": "flupENTHIXOL",
        "fluphenazine": "flupHENAZINE",
        "fluvax": "fluVAx",
        "fluvoxamine": "fluVOXAMine",
        "folic acid": "folic acid",
        "folinic acid": "foliNIc acid",
        "glibenclamide": "gliBENCLAMide",
        "gliclazide": "gliCLAZide",
        "glipizide": "gliPIZide",
        "humalog": "humALOG",
        "humulin": "humULIN",
        "hyoscine butylbromide": "hyoscine BUTYLbromide",
        "hyoscine hydrobromide": "hyoscine HYDRObromide",
        "idarubicin": "IDArubicin",
        "ifosfamide": "IFOSFamide",
        "imipramine": "imipramine",
        "imuprine": "imUPRine",
        "inderal": "INDEral",
        "infliximab": "INFLIximab",
        "isopto carpine": "isopto CARpine",
        "isopto homatropine": "isopto HOMATROpine",
        "isotretinoin": "ISOtretinoin",
        "janumet": "januMET",
        "januvia": "januVIA",
        "ketalar": "ketALAR",
        "ketorolac": "ketOROLAC",
        "lamictal": "laMICTAl",
        "lamisil": "laMISIl",
        "lamivudine": "lamIVUDine",
        "lamotrigine": "lamOTRIGine",
        "lantus": "lanTUs",
        "lanvis": "lanVIs",
        "largactil": "laRGACTIl",
        "leukeran": "LEUKeran",
        "lorazepam": "LORazepam",
        "loxalate": "loxaLATe",
        "loxamine": "loxaMINe",
        "m-enalapril": "m-eNALAPRIL",
        "m-eslon": "m-eSLON",
        "maxidex": "maxiDEX",
        "maxitrol": "maxiTROL",
        "medroxyprogesterone": "meDROXYPROGESTERone",
        "methadone": "methADONe",
        "methotrexate": "metHOTREXATe",
        "methylphenidate": "methYLPHENIDATe",
        "methylprednisolone acetate": "methylprednisolone ACETate",
        "methylprednisolone sodium succinate": "methylprednisolone SODIUM SUCCINate",
        "metoclopramide": "metOCLOPRAMIDe",
        "metoprolol": "metoPROLOL",
        "moxifloxacin": "MOXIfloxacin",
        "myleran": "MYLeran",
        "neo-mercazole": "neO-MERCAZOLe",
        "neoral": "NEOral",
        "neurokare": "neUROKARe",
        "nifedipine": "niFEDIPine",
        "nimodipine": "niMODIPine",
        "nitrazepam": "NITRazepam",
        "norfloxacin": "NORfloxacin",
        "normison": "norMISON",
        "norvasc": "norVASC",
        "novomix": "novoMIX",
        "novorapid": "novoRAPID",
        "novoseven": "novoSEVEN",
        "oxazepam": "OXazepam",
        "oxcarbazepine": "OXCARBazepine",
        "oxycontin": "oxyCONTIN",
        "oxynorm": "oxyNORM",
        "paclitaxel": "PACLItaxel",
        "paroxetine": "PARoxetine",
        "pegfilgrastim": "pegFILGRASTIM",
        "peginterferon": "pegINTERFERON",
        "penicillamine": "penicillAMINE",
        "phenoxymethylpenicillin": "PHENOXYMETHylpenicillin",
        "prednisone": "prEDNISone",
        "primacin": "primaCIN",
        "primacor": "primaCOR",
        "primaquine": "primAQUIne",
        "primaxin": "primaXIN",
        "primidone": "primIDOne",
        "prochlorperazine": "proCHLORPERazine",
        "procyclidine": "procYCLIDine",
        "prograf": "proGRAF",
        "promethazine": "proMETHazine",
        "propofol": "propOFol",
        "propranolol": "propRANOLol",
        "prozac": "proZAC",
        "quetiapine": "QUETIAPine",
        "quinine": "quINine",
        "rituximab": "RITUximab",
        "sertraline": "SERTRALine",
        "sirolimus": "SIrolimus",
        "solu-cortef": "solu-CORTEF",
        "solu-medrol": "SOLU-medrol",
        "sulfadiazine": "sulfaDIazine",
        "sulfasalazine": "sulfaSALazine",
        "tacrolimus": "TACrolimus",
        "temodal": "tEMOdal",
        "tofranil": "toFRANIL",
        "topamax": "toPAMAX",
        "tramadol": "tRAMadol",
        "tretinoin": "tretinoin",
        "trimeprazine": "trimEPRAZINE",
        "trimethoprim": "trimETHOPRIM",
        "trimipramine": "trimIPRAMINE",
        "valaciclovir": "valAciclovir",
        "valganciclovir": "valGANciclovir",
        "vinblastine": "vinBLASTine",
        "vincristine": "vinCRISTine",
        "vinorelbine": "vinORELBine",
    },
}

MAX_WORD_COUNTS: dict[str, int] = {
    "AU": 2,
    "DEFAULT": 2,
    "FDA": 1,
    "ISMP": 2,
    "NZ": 3,
}

VERSIONS: dict[str, str] = {
    "AU": "20171124.0",
    "DEFAULT": "20180819.0",
    "FDA": "20160101.0",
    "ISMP": "20160101.0",
    "NZ": "20131201.0",
}

AVAILABLE_LIST_IDS: frozenset[str] = frozenset(
    {
        "AU",
        "DEFAULT",
        "FDA",
        "ISMP",
        "NZ",
    }
)


def get_list(list_id: str) -> dict[str, str]:
    """Return the casefold->canonical dict for list_id."""
    if list_id in ALL_LISTS:
        return ALL_LISTS[list_id]
    available = ", ".join(sorted(AVAILABLE_LIST_IDS))
    raise TallmanError(f"Unknown Tallman list ID: '{list_id}'. Available lists: {available}.")


def get_max_word_count(list_id: str) -> int:
    """Return the maxWords lookahead cap for list_id (default 1)."""
    return MAX_WORD_COUNTS.get(list_id, 1)


def get_version(list_id: str) -> str:
    """Return the version string for list_id."""
    if list_id in VERSIONS:
        return VERSIONS[list_id]
    available = ", ".join(sorted(AVAILABLE_LIST_IDS))
    raise TallmanError(f"Unknown Tallman list ID: '{list_id}'. Available lists: {available}.")


def has_list(list_id: str) -> bool:
    """Return True if list_id is a known list."""
    return list_id in ALL_LISTS
